...
static int tgc_rehash(tgc_t *gc, size_t new_size) {
  ...
  printf("[DEBUG] resizing pointer table from %zu to %zu slots\n", old_size, new_size);
...
static void tgc_mark_ptr(tgc_t *gc, void *ptr) {
  ...
  printf("[DEBUG] marking pointer %p\n", ptr);
...
static void *tgc_add(
  ...
  if (tgc_resize_more(gc)) {
    printf("[DEBUG] allocated %zu bytes for %p\n", size, ptr);
    tgc_add_ptr(gc, ptr, size, flags, dtor);
    if (!gc->paused && gc->nitems > gc->mitems) {
      printf("[DEBUG] gc cycle triggered\n", size, ptr);
      tgc_run(gc);
      printf("[DEBUG] gc cycle done\n", size, ptr);
    }
    return ptr;
  } else {
    gc->nitems--;
...
void tgc_debug_ptr(tgc_ptr_t *ptr, void (*pretty_print)(tgc_ptr_t*)) {
